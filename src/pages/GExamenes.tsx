...
const ExamGenerator: React.FC = () => {
  const [chatSessions, setChatSessions] = useState<ChatSession[]>(() => {
    const saved = localStorage.getItem('examChatSessions');
    return saved ? JSON.parse(saved) : [{
      ...
      messages: [{
        role: 'bot',
        content:
          '¡Hola! Soy tu asistente experto en generar exámenes escolares. Por favor indícame los datos del examen: nivel educativo (primaria, secundaria, prepa, universidad, maestría, etc.), materia, tema(s), nivel de dificultad, número de preguntas y tipo de preguntas.',
      }],
    }];
  });
  ...
  useEffect(() => {
    localStorage.setItem('examChatSessions', JSON.stringify(chatSessions));
  ...
  const handleChatSubmit = async (e: React.FormEvent) => {
    ...
    const prompt = `
Eres un experto en elaboración de exámenes para diferentes niveles educativos (primaria, secundaria, preparatoria, universidad, maestría, etc.).
Quiero que generes un examen a partir de los siguientes datos:

${chatInput}

Genera el examen con este formato:

1. **Instrucciones generales** (claras para los alumnos, adaptadas al nivel educativo)

2. **Preguntas**:  
– Pregunta 1 (tipo de pregunta)  
 Opciones (si aplica)  
– Pregunta 2 (tipo de pregunta)  
 Opciones (si aplica)   
...
El examen debe ser claro, variado, adaptado al nivel educativo especificado y alineado al tema solicitado. No inventes contenido que no corresponda al tema.
Entrega el examen bien organizado, listo para revisar y aplicar.
    `;
        ...
        prev.map((session) =>
          session.id === currentSessionId
            ? {
                ...
                messages: [
                  ...session.messages,
                  { role: 'bot', content: 'Lo siento, hubo un error al generar el examen. Intenta de nuevo.' },
                ],
              }
            : session
        )
    ...
    <div className="min-h-screen w-screen flex flex-col items-center bg-gradient-to-b from-blue-50 to-gray-100 p-4">
      <div className="w-[80%] max-w-4xl flex flex-col">
        <h2 className="text-2xl sm:text-3xl font-bold mb-4 text-gray-800 text-center">
          Generador de Exámenes Escolares
        </h2>
          ...
          <div className="md:w-3/4 flex flex-col bg-white rounded-lg shadow-lg">
            ...
            <form onSubmit={handleChatSubmit} className="flex items-center p-4 border-t border-gray-200">
              <textarea
                ...
                placeholder="Escribe aquí los datos del examen..."
